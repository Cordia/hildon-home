				   ...
				   GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_HOME_VIEW:
      priv->view = g_value_get_uint (value);
      break;
				   ...
				   GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_HOME_VIEW:
      g_value_set_uint (value, priv->view);
      break;
		 ...
		 gpointer        data)
{
  ...
  cairo_set_source_rgb (cr,
			priv->color.red/65535.0,
			priv->color.green/65535.0,
			priv->color.blue/65535.0);
  cairo_paint (cr);
  cairo_destroy (cr);
...
/*      ClutterColor       color; */
	...
	GTK_COLOR_SELECTION (GTK_COLOR_SELECTION_DIALOG (dialog)->colorsel);
      ...
      gtk_color_selection_get_current_color (color_sel, &priv->color);
			 ...
			 HdBackgroundDialog *parent)
{
  ...
  color_sel =
    GTK_COLOR_SELECTION (GTK_COLOR_SELECTION_DIALOG (dialog)->colorsel);
  gtk_color_selection_set_current_color (color_sel, &priv->color);
...
GtkWidget *
hd_background_dialog_new (guint view)
{
